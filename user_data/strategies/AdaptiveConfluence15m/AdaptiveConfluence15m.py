...
class AdaptiveConfluence15m(IStrategy):
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "15m"
    ...
    rsi_min_breakout = IntParameter(40, 55, default=45, space="buy", optimize=True)
    rsi_max_breakout = IntParameter(70, 90, default=82, space="buy", optimize=True)
    volume_min_mult = DecimalParameter(
        0.8, 2.0, default=1.2, decimals=1, space="buy", optimize=True
    )
    body_ratio_min = DecimalParameter(0.3, 0.7, default=0.5, decimals=2, space="buy", optimize=True)
    ...
    atr_stop_mult_pullback = DecimalParameter(
        0.2, 0.8, default=0.4, decimals=1, space="buy", optimize=True
    )

    # ── sell space ─────────────────────────────────────────────
    momentum_exit_rsi_peak = IntParameter(60, 80, default=70, space="sell", optimize=True)
    momentum_exit_rsi = IntParameter(55, 70, default=60, space="sell", optimize=True)
    macd_decline_bars = IntParameter(2, 5, default=3, space="sell", optimize=True)
    time_stop_breakout = IntParameter(8, 20, default=12, space="sell", optimize=True)
    time_stop_pullback = IntParameter(5, 15, default=8, space="sell", optimize=True)
    ...
    def informative_pairs(self):
        ...
        for pair in self.dp.current_whitelist():
            ...
            pairs.append((pair, "1d"))
        return pairs

    # ── 1d indicators: trend direction ─────────────────────────
    ...
    # ── 4h indicators: swing points + volatility regime ────────
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if inf_1d is not None and not inf_1d.empty:
            inf_1d = self.populate_indicators_1d(inf_1d.copy(), metadata)
            inf_1d["_merge_key"] = inf_1d["date"] + pd.Timedelta(days=1) - pd.Timedelta(minutes=15)
            dataframe = dataframe.merge(
                inf_1d[["_merge_key", "trend_up", "trend_down", "adx"]].rename(
                    columns={
                        "trend_up": "trend_up_1d",
                        "trend_down": "trend_down_1d",
                        "adx": "adx_1d",
                    }
                ),
                left_on="date",
            ...
            dataframe["trend_up_1d"] = dataframe["trend_up_1d"].ffill().fillna(0)
            dataframe["trend_down_1d"] = dataframe["trend_down_1d"].ffill().fillna(0)
            dataframe["adx_1d"] = dataframe["adx_1d"].ffill()
        else:
            dataframe["trend_up_1d"] = 0
            dataframe["trend_down_1d"] = 0
            dataframe["adx_1d"] = np.nan
        ...
        if inf_4h is not None and not inf_4h.empty:
            ...
            inf_4h["_merge_key"] = inf_4h["date"] + pd.Timedelta(hours=4) - pd.Timedelta(minutes=15)
            merge_cols = [
                "_merge_key",
                "atr_pct_rank",
                "vol_expanding",
            ...
            dataframe = dataframe.merge(
                inf_4h[merge_cols].rename(
                    columns={
                        "atr_pct_rank": "atr_pct_rank_4h",
                        "vol_expanding": "vol_expanding_4h",
                        "resistance": "resistance_4h",
                        "support": "support_4h",
                        "proximity": "proximity_4h",
                        "swing_high": "swing_high_4h",
            ...
            dataframe["atr_pct_rank_4h"] = np.nan
            dataframe["vol_expanding_4h"] = 0
            dataframe["resistance_4h"] = np.nan
            dataframe["support_4h"] = np.nan
            dataframe["proximity_4h"] = np.nan
            dataframe["swing_high_4h"] = np.nan
            dataframe["swing_low_4h"] = np.nan

        # ── volatility regime ──────────────────────────────────
        dataframe["vol_regime"] = "normal"
        pct = dataframe["atr_pct_rank_4h"]
    ...
    def _build_entry_signals(self, df: DataFrame) -> DataFrame:
        ...
        trend_up = df["trend_up_1d"] == 1
        trend_down = df["trend_down_1d"] == 1
        trend_neutral = (df["trend_up_1d"] == 0) & (df["trend_down_1d"] == 0)

        vol_low = df["vol_regime"] == "low"
        vol_expanding = df["vol_expanding_4h"] == 1
        vol_not_low = ~vol_low
        ...
        resistance = df["resistance_4h"]
        support = df["support_4h"]
    ...
    def leverage(
        ...
        **kwargs: Any,
        ...
        trend_neutral = (dataframe.iloc[-1].get("trend_up_1d", 0) == 0) & (
            dataframe.iloc[-1].get("trend_down_1d", 0) == 0
        )
        if trend_neutral:
    ...
    def custom_stoploss(
        ...
        **kwargs: Any,
        ...
        if is_breakout:
            stop_distance = self.atr_stop_mult_breakout.value * atr
        elif is_pullback:
            stop_distance = self.atr_stop_mult_pullback.value * atr
        else:
            stop_distance = self.atr_stop_mult_breakout.value * atr
    ...
    def custom_exit(
        ...
        **kwargs: Any,
        ...
        candle = dataframe.iloc[-1]
        hold_candles = (current_time - trade.open_date_utc).total_seconds() / 900.0
        is_breakout = "breakout" in (trade.enter_tag or "")
        is_pullback = "pullback" in (trade.enter_tag or "")
    ...
    def _check_trend_reversal(self, candle: pd.Series, is_short: bool) -> str | None:
        trend_up = candle.get("trend_up_1d", 0) == 1
        trend_down = candle.get("trend_down_1d", 0) == 1
    ...
    def _check_sr_target(
        ...
        if is_short:
            target = candle.get("support_4h", np.nan)
            if not pd.isna(target) and current_rate <= target:
                return "sr_target"
        else:
            target = candle.get("resistance_4h", np.nan)
            if not pd.isna(target) and current_rate >= target:
                return "sr_target"
        return None
    ...
    def custom_stake_amount(
        ...
        **kwargs: Any,
        ...
        if vol_regime == "high" or vol_regime == "low":
            stake *= 0.7

        trend_neutral = (candle.get("trend_up_1d", 0) == 0) & (candle.get("trend_down_1d", 0) == 0)
        if trend_neutral:
            stake *= 0.5
