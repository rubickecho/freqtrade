...
class TrendFlowV3(IStrategy):
    ...
    ema_period = IntParameter(20, 60, default=39, space="buy", optimize=True)
    adx_threshold = IntParameter(15, 30, default=17, space="buy", optimize=True)
    atr_stop_mult = DecimalParameter(1.5, 3.0, default=1.5, decimals=1, space="sell", optimize=True)
    ...
    def custom_stoploss(
        ...
        **kwargs: Any,
        ...
        stop_price_pct = (stop_mult * atr) / trade.open_rate
        stop_price = trade.open_rate * (
            (1 + stop_price_pct) if trade.is_short else (1 - stop_price_pct)
        )
